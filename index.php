    ...
    <div class="container-fluid page">
        <div class=" container row mb-4">
                ...
                <div class="row p-2">
                    ...
                    if ($response) {
                        while ($data = $response->fetch_assoc()) {
                            $id = $data['id'];
                            $category = $data['book_category'];
                            echo "<div class='col-md-3 contributebook-view mb-3' bookid='" . $id . "' email='" . $username . "' category='" . $category . "' >
                               <div class='card border-0 p-0'>
                                   <div class='card-body p-1'>";
                            $image = "data:image/png;base64," . base64_encode($data['book_image']);
                            $book_title = $data['book_title'];
            ...
            <div class="col-md-2">
                ...
                <hr class="style-one">
                <?php
                $get_max_freq_book = "SELECT id FROM sellbook where totalsell=(SELECT MAX(totalsell) FROM sellbook)";
                $response = $db->query($get_max_freq_book);
                $data = $response->fetch_assoc(); 
                $bookid = $data['id'];
                
                $flag = 0;
                $recommend_pepole = "SELECT * FROM buy";
                $response = $db->query($recommend_pepole);
                
                while($fetch_data= $response->fetch_assoc())
                {
                    if($fetch_data['bookid']==$bookid && $fetch_data['email']==$username)
                    {
                        $flag =1;
                        break;
                    }
